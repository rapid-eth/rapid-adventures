...
/* --- Component --- */
export const useSet= (state, dispatch) => {
			...
			switch (selected.type) {
					...
					if (selected) {
						const runEffect = async () => {
							...
							try {
								if (space) {
									if (append) {
										const data = await state.spaces[
											space
										].instance[access].get(append);
										if (data) {
											/**
											 * IF : key
											 * TRUE : Update object
										...
										await state.spaces[space].instance[
											access
										].setMultiple(keys, inputs);
										dispatch({
											type: 'SET_SUCCESS'
										});
										if (selected.update) {
									...
									if (append) {
										const data = await state.auth.instance[
											access
										].get(append);
										if (data) {
										...
										Array.isArray(data)
											? await state.instance[access].set(
													append,
													listUpdated
											  )
				...
				case 'SET_INSERT_REQUEST':
					...
					try {
						...
						state.instance[selected.access].set(
							selected.index,
							data
						);
						dispatch({
				...
				case 'SET_MERGE_REQUEST':
					...
					try {
						if (selected.space) {
							 ...
							 */
							console.log(
								state.auth.spaces[selected.space],
								'uessss'
							);
							state.auth.spaces[selected.space].instance[
								selected.access
							]

...
/* --- Component --- */
export const useThreadPost = (state, dispatch) => {
	...
	useEffect(() => {
		if (state.store && state.store.posts) {
			try {
				...
				if (
					postSelected &&
					state.auth.threads[postSelected.threadName].instance
				) {
					const runEffect = async () => {
						...
						switch (postSelected.type) {
							case 'THREAD_POST_PUBLISH_REQUEST':
								...
								].instance.post(postSelected.post);
								posts = await state.auth.threads[
									postSelected.threadName
								].instance.getPosts();
								dispatch({
							...
							case 'THREAD_POST_DELETE_REQUEST':
								if (postSelected.postId) {
									...
									await state.auth.threads[
										postSelected.threadName
									].instance.deletePost(postSelected.postId);
									posts = await state.auth.threads[
										postSelected.threadName
									].instance.getPosts();
									dispatch({
